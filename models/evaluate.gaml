...
//	Extension2
	map<road, float> update_speed update: road as_map(each::each.shape.perimeter/each.speed_rate);
...
species road{
	...
	int num_driver <- 0 update: length(people at_distance(2#m));
	float speed_rate <- 1.0 update: exp(-num_driver/capacity) min:0.1;
	
	aspect default{
		draw (shape buffer 1+ 5*(1-speed_rate)) color: #blue;
	}
}
...
//				write("observe and random search");
	...
	aspect default{
		if mobility_choice = CAR{
			draw square(13) color: is_informed? #red:#purple ;
		}else if mobility_choice = MOTO{
			draw triangle(16) color: is_informed? #red:#purple ;
		}else{
			draw circle(8) color: is_informed? #red:#purple ;
		}
	}
}
